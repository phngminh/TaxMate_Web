      ...
      <div className='bg-white rounded-[16px] shadow-2xl max-w-xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200'>
        ...
        >
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (clean === '') {
                    ...
                    setProductForm(prev => ({ ...prev, costPrice: parseInt(clean).toLocaleString('vi-VN') }))
                  }
                }}
                className='w-full border border-gray-200 rounded-[8px] px-3.5 py-2 text-[13.5px] outline-hidden focus:border-[#D32F2F] transition-all font-medium text-gray-800 text-right'
              />
          ...
          {isEditing && isProduct && isStockTrackingEnabled && (
            <div className='p-3 bg-blue-50/80 border border-blue-200 rounded-lg text-blue-800 text-[12.5px] font-medium'>
              Tồn kho và giá vốn được cập nhật tại màn Khởi tạo / kiểm kê tồn kho.
            </div>
          )}
